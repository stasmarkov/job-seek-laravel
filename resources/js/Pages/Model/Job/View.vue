...
const like = () => {
  Inertia.get(route('job.like', {'job': props.job.id}));
}
</script>

<template>
  <Head :title="props.job.title">
    <meta name="description" :content="props.job.short_description">
  </Head>

  <Layout>
    <div v-if="can.edit_job" class="flex gap-2 mb-2">
      <LinkButton class="bg-green-600" :href="route('job.edit', { job: props.job.id })">Edit</LinkButton>
    </div>
    ...
    <Article :hoverable="false">
        ...
        <div class="px-4 sm:px-0">
          <div class="flex justify-between">
            <h3 class="text-base font-semibold leading-7">
              ...
              <span class="text-3xl font-bold">{{ props.job.title }}</span>
            </h3>
            <EmployerProfileLogo :employerProfile="props.job.employerProfile" width="50"/>
          </div>

          <p class="mt-1 max-w-2xl text-sm leading-6 text-gray-500">Employer:
            <Link v-if="props.job.employerProfile"
                  class="self-start text-sm text-gray-200 hover:underline hover:text-blue-500 ease-linear transition"
                  :href="route('profile.employer.show', { employerProfile: props.job.employerProfile.id })">
              {{ props.job.employerProfile.name }}
            </Link>
          </p>
        </div>
        <div class="mt-6 border-t border-gray-100">
          <dl class="divide-y divide-gray-100">
            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <dd class="mt-1 text-sm leading-6 text-gray-300 sm:col-span-2 sm:mt-0">{{ job.salary }}</dd>
            </div>
            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              <dt class="text-sm font-medium leading-6 text-gray-100">Schedule</dt>
              <dd class="mt-1 text-sm leading-6 text-gray-300 sm:col-span-2 sm:mt-0">{{ props.job.schedule }}</dd>
            </div>
            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              <dt class="text-sm font-medium leading-6 text-gray-100">Location</dt>
              <dd class="mt-1 text-sm leading-6 text-gray-300 sm:col-span-2 sm:mt-0">{{ props.job.location }}</dd>
            </div>
            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
            ...
            <div class="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <dd class="mt-2 text-sm text-gray-100 sm:col-span-2 sm:mt-0">
                <div class="flex flex-wrap mt-auto gap-2">
                  <Tag v-for="tag in props.job.tags" size="small" :tag/>
                </div>
              </dd>
            </div>
          </dl>
      ...
      <div class="mt-4 flex justify-end">
        <Like :route="route('job.like', {'job': props.job.id })" :likesCount="props.likesCount"
              :isLiked="props.isLiked"/>
        <AnchorButton :href="props.job.url" target="_blank">View more</AnchorButton>
      </div>
    </Article>

    <div class="space-y-4 mt-6">
      <BlockHeading>Similar jobs</BlockHeading>
      <CardWide v-for="similarJob in props.similarJobs" :job="similarJob" />
    </div>
