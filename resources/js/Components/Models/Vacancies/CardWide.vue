...
<template>
  <Panel class="flex gap-x-6 flex-wrap bg-black text-white max-w-5xl mx-auto">
    ...
    <div class="flex-1 flex flex-col md:flex-1 gap-0.5">
      <span class="text-sm text-gray-300"><RegularLink v-if="vacancy.employerProfile" class="self-start" :href="route('profile.employer.show', { employerProfile: vacancy.employerProfile.id })">{{ vacancy.employerProfile.name }}</RegularLink></span>
